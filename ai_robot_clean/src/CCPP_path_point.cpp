...
private:
    ...
    void pose_callback(const geometry_msgs::PoseStamped &poses);
...
CCPP_path_point::CCPP_path_point(ros::NodeHandle &nh)
{
    ...
    subOdom = nh.subscribe("/est_pose", 1000, &CCPP_path_point::pose_callback, this);
...
void CCPP_path_point::pose_callback(const geometry_msgs::PoseStamped &poses) {
    passed_path.header = poses.header;
    x_current = poses.pose.position.x;
    y_current = poses.pose.position.y;
