...
//        cout<<"movebyhand:"<<movecmd[0]<<","<<movecmd[1]<<endl;
    ...
    if(fromOrderToTarget(order_hflrb, target_pos)){
        double vx = 0., rz = 0.;
        if(turnback_cnt > 1){
            ROS_INFO("first rz in turnback_cnt is: %f", rz);
            if(turnback_cnt == 30){
            ...
            lastvx = vx;
            lastrz = rz;
            ROS_INFO("second rz in turnback_cnt is: %f", rz);
        }
        else{
            // find path with dwa
            bool forwardok = simpleDWA(target_pos, vx, rz);//加入加计时器，
            if(!forwardok){
                t2 = boost::chrono::system_clock::now();
                time_used += boost::chrono::duration_cast<boost::chrono::duration<double>>(t2-t1);
            }
            if(forwardok && success_time_used.count() <= 0.1)
                time_used = boost::chrono::seconds::zero();
            if(!forwardok){
                turnback_cnt = 30;
            }
        }
        if(!test_flag && !lock_moving){
            rob_ctrl.move(vx, rz);
            success_t = boost::chrono::system_clock::now();
            success_time_used = boost::chrono::duration_cast<boost::chrono::duration<double>>(success_t - last_success_t);
            last_success_t = success_t;
        }
        //// todo 转弯、掉头，累计超时掉头,vx rz rob_ctrl.move(vx, rz);  可以考虑使用前面的30 step来做速度平滑
        if(backward_flag) {
            ROS_INFO("**************************backward is true*********************");
            double rad = 3.1415, gol = 0.;
            while (rad >= 0.0) {
//                    calCmd(vx, rz, rad, 0);
                rz = 3.1415 / 8;
                vx = 0;
                ...
                rad = rad - rz * 1.0;
                ROS_INFO("rz is running, and the lasted rad is: %f", rad);
            }
            rob_ctrl.move(0., 0.);
                while(gol >= 0.0){
                    vx = 0.25;
...
//                    calCmd(vx, rz, 0, gol);
                    ...
                    ros::Duration(1.0).sleep();
                    gol = gol - vx * 1.0;
                    ROS_INFO("gol is: %f", gol);
                }
            rob_ctrl.move(0., 0.);
            backward_flag = false;
        }
        ROS_INFO("*******Continuing seplan!*******");
    }
}
...
void SEPlanner::readParam(){
    ...
    nh.param("seleplanner/backward_flag", backward_flag, false);
...
void SEPlanner::initSubPub(){
    ...
    odom_sub = nh.subscribe("/sim_p3at/odom", 1, &SEPlanner::odomCB, this);
...
//        cout<<"dist to target:"<<dist_2<<";"<<this_pos<<","<<target<<endl;
    ...
    double dist = sqrt(pow((target[0]-dwa_path_points[i_rad][step][0]),2) + pow((target[1]-dwa_path_points[i_rad][step][1]),2));//dist 使用类似A*替换
...
void SEPlanner::odomCB(const nav_msgs::OdometryConstPtr &msg) {
    if(time_used.count() <= 3.5 && time_used.count() >= 0.0){
        ROS_INFO("timed_used is: %f", time_used.count());
        pose_pair.first = msg->pose.pose.position.x;
        pose_pair.second = msg->pose.pose.position.y;
        deque_pose.push_back(pose_pair);
    }
    else if(time_used.count() > 3.5){
        deltx = deque_pose.front().first - deque_pose.back().first;
        ROS_INFO("deltx is: %f", deltx);
        delty = deque_pose.front().second - deque_pose.back().second;
        ROS_INFO("delty is: %f", delty);
        double deltS = sqrt(pow(deltx, 2) + pow(delty, 2));
        ROS_INFO("the distance between the last and first point is: %f", deltS);
        if (deltS < 0.3)
            backward_flag = true;
        deque_pose.clear();
        ROS_INFO("");
